...
Path('tmp/C_location').mkdir(exist_ok=True)
...
def pmap(arg):
    ...
    with open(f'tmp/C_location/{ii:03d}.pkl', 'wb') as fp:
        pickle.dump((labels, lil[:len(labels)]), fp)

args = [(ii, path) for ii, path in enumerate(sorted(Path().glob('tmp/B_location/*.csv')))]
