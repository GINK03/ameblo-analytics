...
TL += ['ワープア', '非正規', '派遣切り', '派遣']
TL += ['タピオカ']
TL += ['壁ドン', '生活保護', 'ナマポ', 'youtube']
TL += ['pm2.5', 'アベノミクス', 'ブラック企業', 'ヘイトスピーチ', 'ご当地キャラ'] # 2013
TL += ['爆買い', 'アベ政治を許さない', '一億総活躍社会', 'ドローン', 'トリプルスリー'] # 2015
TL += ['聖地巡礼', 'トランプ', 'ppap', 'ポケモンgo'] # 2016
TL += ['インスタ映え', 'おっさんずラブ', 'eスポーツ', '災害級'] # 2018 - 2019
TL += ['子宮頸がん', '子宮頸がんワクチン', 'テレビ', '新聞', '芸能人']
TL += ['ユーチューバ']
TL += ['ガン', '闘病', 'うつ病']
...
if '--agg' in sys.argv:
    ...
    dfContent = [pd.read_pickle(path)
                 for path in list(Path('./tmp').glob('content_*.pkl'))]
    dfContent = pd.concat(dfContent, axis=0)
    print('laod dataset')
    df = dfContent.join(dfProfile.set_index('user'), on='user')
    df.drop_duplicates(subset="canonical", keep=False, inplace=True)
    ...
    for tar in TL: # + ['age']:
        ...
        ax.set_ylabel(f'{tar}_rate', fontsize=40)
