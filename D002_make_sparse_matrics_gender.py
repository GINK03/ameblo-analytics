...
def pmap(arg):
    ...
    with open(f'tmp/C_gender/{ii:03d}.pkl', 'wb') as fp:
        pickle.dump((labels, lil[:len(labels)]), fp)
        # print(label, body)

Path('tmp/C_gender').mkdir(exist_ok=True)
args = [(ii, path) for ii, path in enumerate(sorted(Path().glob('tmp/B_gender/*.csv')))]
